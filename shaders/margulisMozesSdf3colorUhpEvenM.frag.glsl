// Margulis-Mozes Theorem 5 tiling — UHP model, polygon SDF edges, 3-coloring.
//
// EVEN branching factor m (2, 4, 6, ...).
// Uses the digit-parity-transition algorithm, dual to the odd-m
// AlphaEvolve formula (which counts odd digits).
//
// For l ∈ Z with floor-base-m expansion d_0, d_1, ..., d_{k-1} and
// terminator ε ∈ {0, -1}, extend to a parity sequence
//   p_i = d_i mod 2     for i < k,
//   p_i = |ε|           for i ≥ k        (since m even, (m-1) is odd, so
//                                         the tail parity equals |ε|).
// Let T(l) = #{i ≥ 0 : p_i ≠ p_{i+1}} be the number of parity transitions
// in the parity sequence. Then
//   color(h, l) = (h + T(l) + |ε(l)|) mod 3.
//
// Companion to margulisMozesSdf3colorUhp.frag.glsl (odd m).
...
// 3-coloring via base-m digit-parity transition count (m must be even).
// prev_parity = -1 is a sentinel meaning "no previous digit seen yet";
// we skip the in-loop transition check on the first digit and skip the
// final tail transition when no digits were processed at all (l = 0 or -1).
int getColor(int h, int l, int N){
  ...
  for(int i = 0; i < 40; i++){
    ...
    int parity = remainder & 1;
    if(prev_parity >= 0 && parity != prev_parity) t_count++;
    prev_parity = parity;
    current_l = quotient;
  }

  int eps_abs = (current_l == -1) ? 1 : 0;
  int tail_parity = eps_abs;  // m even ⇒ (m-1) odd ⇒ tail parity = |ε|

  // Transition from the last actual digit to the tail.
  if(prev_parity >= 0 && prev_parity != tail_parity) t_count++;

  int result = h + t_count + eps_abs;
...
void main(){
  float m  = max(uChildren, 2.0);  // even m: 2, 4, 6, ...
