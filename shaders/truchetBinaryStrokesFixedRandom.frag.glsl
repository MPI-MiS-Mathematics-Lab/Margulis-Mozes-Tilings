...
// Coordinates are evaluated in canonical-tile UHP coordinates (x, y) = (u, 1+v).
...
float hypBezierDistTile(vec2 pUv, vec2 p0Uv, vec2 p1Uv, vec2 p2Uv, vec2 p3Uv) {
  const int STEPS = 36;
  vec2 pUhp = vec2(pUv.x, 1.0 + pUv.y);
  vec2 p0 = vec2(p0Uv.x, 1.0 + p0Uv.y);
  vec2 p1 = vec2(p1Uv.x, 1.0 + p1Uv.y);
  vec2 p2 = vec2(p2Uv.x, 1.0 + p2Uv.y);
  vec2 p3 = vec2(p3Uv.x, 1.0 + p3Uv.y);
...
float bezierConnectorDist(vec2 p, vec2 a, vec2 na, vec2 b, vec2 nb) {
  vec2 aUhp = vec2(a.x, 1.0 + a.y);
  vec2 bUhp = vec2(b.x, 1.0 + b.y);
  ...
  float eps = 1e-3;
  vec2 aD2 = uhpToDisk(vec2(a.x + eps * na.x, 1.0 + a.y + eps * na.y));
  vec2 bD2 = uhpToDisk(vec2(b.x + eps * nb.x, 1.0 + b.y + eps * nb.y));
  ...
  vec2 p1 = vec2(p1Uhp.x, p1Uhp.y - 1.0);
  vec2 p2 = vec2(p2Uhp.x, p2Uhp.y - 1.0);
  return hypBezierDistTile(p, a, p1, p2, b);
}

void evalHyperbolicConnector(
  vec2 p, vec2 a, vec2 na, vec2 b, vec2 nb, float id,
  inout float bestD, inout float bestConn
) {
...
void truchetClosestInfo(vec2 uv, int t, out float d, out float tCurve, out float connId, out vec2 tan) {
  ...
  connId = 0.0;
  tan = vec2(1.0, 0.0);
  // 3 motifs with hyperbolic connectors.
  if (t == 0) {
    evalHyperbolicConnector(uv, SL, NSL, CL, NCL, 0.0, d, connId);
    evalHyperbolicConnector(uv, P, NP, SR, NSR, 1.0, d, connId);
  } else if (t == 1) {
    evalHyperbolicConnector(uv, P, NP, SL, NSL, 2.0, d, connId);
    evalHyperbolicConnector(uv, SR, NSR, CR, NCR, 3.0, d, connId);
  } else {
    evalHyperbolicConnector(uv, SL, NSL, CL, NCL, 4.0, d, connId);
    evalHyperbolicConnector(uv, SR, NSR, CR, NCR, 5.0, d, connId);
  }
}
...
vec3 shadeBinaryBand(vec2 s, vec2 zeta, float x, float y, float nBand, float smh) {
  ...
  int tileId = int(floor(3.0 * hash12(vec2(nBand + 37.0, j - 91.0))));
  ...
  float widthHyp = STROKE_RHO * 1.75;
  float edge = dCenter - widthHyp;
  ...
  vec2 uvUhp = vec2(uv.x, 1.0 + uv.y);
  vec2 capUhp = vec2(capCenter.x, 1.0 + capCenter.y);
  float dCap = hypDistUhp(uvUhp, capUhp) - widthHyp;
  float joinK = 0.75 * widthHyp;
...
void main() {
  ...
  vec3 col = renderBinaryTruchet(s);
