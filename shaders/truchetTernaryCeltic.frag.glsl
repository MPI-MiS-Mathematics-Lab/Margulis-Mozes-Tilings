...
float bezierConnectorDist(vec2 p, vec2 a, vec2 na, vec2 b, vec2 nb) {
  ...
  vec2 p1 = canonicalUhpToTileUv(p1Uhp);
  vec2 p2 = canonicalUhpToTileUv(p2Uhp);
...
float countResidueRange(float a, float b, float residue) {
  if (b < a) return 0.0;
  float first = a + modPos(residue - a, 3.0);
  if (first > b) return 0.0;
  return floor((b - first) / 3.0) + 1.0;
}

float ternaryHorizontalPhaseCount(float h, float l) {
  float residue = modPos(h, 3.0);
  if (l > 0.5) return countResidueRange(1.0, l, residue);
  if (l < -0.5) return countResidueRange(l + 1.0, 0.0, residue);
  return 0.0;
}

float ternaryVerticalClPhaseCount(float h) {
  if (h > 0.5) return countResidueRange(1.0, h, 1.0);
  if (h < -0.5) return countResidueRange(h + 1.0, 0.0, 1.0);
  return 0.0;
}

float ternaryWeavePhase(float h, float l) {
  const float BASE_PHASE = 0.0;
  float flips = ternaryHorizontalPhaseCount(h, l) + ternaryVerticalClPhaseCount(h);
  return mod(BASE_PHASE + flips, 2.0);
}

vec3 truchetTernaryConnectorDistances(vec2 uv, int t) {
  ...
  // Requested motifs:
  // t=0: (P-CC), (SL-SR), (CL-CR)
  // t=1: (P-SR), (SL-CC), (CL-CR)
  // t=2: (P-SL), (SR-CC), (CL-CR)
  if (t == 0) {
    return vec3(
      bezierConnectorDist(uv, P, NP, CC, NCC),
      bezierConnectorDist(uv, SL, NSL, SR, NSR),
      bezierConnectorDist(uv, CL, NCL, CR, NCR)
    );
  } else if (t == 1) {
    return vec3(
      bezierConnectorDist(uv, P, NP, SR, NSR),
      bezierConnectorDist(uv, SL, NSL, CC, NCC),
      bezierConnectorDist(uv, CL, NCL, CR, NCR)
    );
  } else {
    return vec3(
      bezierConnectorDist(uv, P, NP, SL, NSL),
      bezierConnectorDist(uv, SR, NSR, CC, NCC),
      bezierConnectorDist(uv, CL, NCL, CR, NCR)
    );
  }
}
...
float truchetTernaryDistance(vec2 uv, int t) {
  vec3 d = truchetTernaryConnectorDistances(uv, t);
...
vec3 applyCelticRibbon(vec3 col, float outerMask, float innerMask) {
  col = mix(col, vec3(0.08), outerMask);
  col = mix(col, vec3(0.88), innerMask);
  return col;
}
...
vec3 shadeTernaryBand(float x, float y, float nBand, float smh) {
  ...
  vec3 d = truchetTernaryConnectorDistances(uv, tileId);
  float d0 = d.x;
  float d1 = d.y;
  float d2 = d.z;

  float widthHyp = STROKE_RHO * 1.14;
  float innerW = 0.56 * widthHyp;
  float o0 = 1.0 - step(0.0, d0 - widthHyp);
  float o1 = 1.0 - step(0.0, d1 - widthHyp);
  float o2 = 1.0 - step(0.0, d2 - widthHyp);
  float i0 = 1.0 - step(0.0, d0 - innerW);
  float i1 = 1.0 - step(0.0, d1 - innerW);
  float i2 = 1.0 - step(0.0, d2 - innerW);

  vec3 col = vec3(1.0);

  // Tile-specific layering for t0:
  // d0=P-CC above d1=SL-SR, and d0 below d2=CL-CR.
  if (tileId == 0) {
    float weavePhase = ternaryWeavePhase(nBand, j);
    float cross01 = 1.0 - smoothstep(0.0, 0.36 * widthHyp, max(d0, d1));
    float cross02 = 1.0 - smoothstep(0.0, 0.36 * widthHyp, max(d0, d2));
    if (weavePhase < 0.5) {
      // P-CC over SL-SR, and under CL-CR.
      float oBottom = o1 * (1.0 - cross01);
      float iBottom = i1 * (1.0 - cross01);
      float oMid = o0 * (1.0 - cross02);
      float iMid = i0 * (1.0 - cross02);
      col = applyCelticRibbon(col, oBottom, iBottom);
      col = applyCelticRibbon(col, oMid, iMid);
      col = applyCelticRibbon(col, o2, i2);
    } else {
      // P-CC under SL-SR, and over CL-CR.
      float oBottom = o2 * (1.0 - cross02);
      float iBottom = i2 * (1.0 - cross02);
      float oMid = o0 * (1.0 - cross01);
      float iMid = i0 * (1.0 - cross01);
      col = applyCelticRibbon(col, oBottom, iBottom);
      col = applyCelticRibbon(col, oMid, iMid);
      col = applyCelticRibbon(col, o1, i1);
    }
  } else {
    // Default alternating over/under for the other motifs (d0 with d1 crossing pair).
    float overSwap = mod(floor(nBand) + floor(j), 2.0);
    float oOver = mix(o0, o1, overSwap);
    float iOver = mix(i0, i1, overSwap);
    float oUnder = mix(o1, o0, overSwap);
    float iUnder = mix(i1, i0, overSwap);
    float crossMask = 1.0 - smoothstep(0.0, 0.36 * widthHyp, max(d0, d1));

    oUnder *= (1.0 - crossMask);
    iUnder *= (1.0 - crossMask);

    col = applyCelticRibbon(col, oUnder, iUnder);
    col = applyCelticRibbon(col, o2, i2);
    col = applyCelticRibbon(col, oOver, iOver);
  }
...
void main() {
  ...
  vec3 col = renderTernaryTruchet(s);
