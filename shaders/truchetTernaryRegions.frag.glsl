...
float ternaryHorizontalFlipCount(float h, float l) {
  // Sibling edge flips happen when crossing an edge whose left motif is t1.
  // With tileId(h, k) = h + 2k mod 3, that occurs when k = h mod 3.
...
float ternaryVerticalClFlipCount(float h) {
  // Along the l=0 (cl) chain, parent->child flips only when parent motif is t0.
  // That is equivalent to child level k = 1 mod 3.
...
float ternaryTileFlip(float h, float l) {
  const float BASE_FLIP = 1.0; // choice at tile (0,0)
  float flips = ternaryHorizontalFlipCount(h, l) + ternaryVerticalClFlipCount(h);
  return mod(BASE_FLIP + flips, 2.0);
}
