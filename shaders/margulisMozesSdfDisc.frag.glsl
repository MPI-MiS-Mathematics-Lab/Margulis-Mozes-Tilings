// Margulis-Mozes Theorem 5 tiling — Poincaré disc model, polygon SDF edges.
//
// Maps from disc to UHP via Cayley transform, then runs the tiling algorithm
// with unified polygon SDF for edge rendering.
...
// ─── Polygon SDF in local UHP coords ────────────────────────────────────────
...
void main(){
  ...
  if(r2 < 1.0){
    // Poincaré disc → UHP via Cayley transform
    vec2 z   = s;
    ...
    // Hyperbolic zoom: z → z/zoom (isometry, shifts which level is at disc center)
